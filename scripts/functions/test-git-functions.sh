...
/bin/bash ./scripts/v2/githooks/git-branch-commit-msg.sh
if [[ $? = 1 ]]; then
    all_pass=1
fi
...
/bin/bash ./scripts/v2/githooks/git-commit-msg.sh  "My valid commit message"
if [[ $? = 0 ]]; then
    all_pass=1
fi
...
/bin/bash ./scripts/v2/githooks/git-commit-msg.sh
if [[ $? = 0 ]]; then
    all_pass=1
fi
...
/bin/bash ./scripts/v2/githooks/git-commit-msg.sh  "DR-11 My message"
if [[ $? = 0 ]]; then
    all_pass=1
fi
